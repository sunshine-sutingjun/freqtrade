...
function ensure_uv() {
    if ! command -v uv >/dev/null 2>&1; then
        echo_block "Installing uv (universal virtualenv/pip)"
        # Try installer script from Astral
        curl -fsSL https://astral.sh/uv/install.sh | sh
        # Ensure common install paths are on PATH for this script
        export PATH="$HOME/.local/bin:$HOME/.cargo/bin:$PATH"
        if ! command -v uv >/dev/null 2>&1; then
            echo "uv installation not found on PATH. Please add ~/.local/bin to PATH and rerun."
            exit 1
        fi
    fi
}
...
function updateenv() {
    ...
    SYS_ARCH=$(uname -m)
    echo "uv pip install in-progress. Please wait..."
    ...
    then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]
        then
            # uv builds using PEP 517 by default; no --use-pep517 needed
    ...
    uv pip install --upgrade -r ${REQUIREMENTS} ${REQUIREMENTS_HYPEROPT} ${REQUIREMENTS_PLOT} ${REQUIREMENTS_FREQAI} ${REQUIREMENTS_FREQAI_RL}
    if [ $? -ne 0 ]; then
...
function recreate_environments() {
    ...
    echo
    # Prefer uv for creating the virtual environment
    ensure_uv
    uv venv --python ${PYTHON} .venv
    if [ $? -ne 0 ]; then
        echo "uv failed to create virtual environment, falling back to python -m venv"
        ${PYTHON} -m venv .venv
        if [ $? -ne 0 ]; then
